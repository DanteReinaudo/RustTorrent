...
impl Logger {
    ...
    pub fn new(
        ...
        file_name: &str,
        rx: Receiver<String>,
    ) -> Result<Logger, BitTrackerError> {
        let folder = Path::new(log_path);
        if !folder.is_dir() {
            let _f = create_dir(folder).or(Err(BitTrackerError::FileCreationError))?;
        }

        let path_name = log_path.to_string() + "/" + file_name + ".log";
        let path = Path::new(&path_name);
        if !path.exists() {
            let _f = File::create(path).or(Err(BitTrackerError::FileCreationError))?;
        }
    ...
    fn write_log(&mut self, message: &str) -> Result<bool, BitTrackerError> {
        let time = Local::now().to_string();
        let log_message = Self::make_message(&time, message);
        writeln!(self.file, "{}", log_message).or(Err(BitTrackerError::WriteFileError))?;
        Ok(true)
    }
    ...
    pub fn listening(&mut self) -> Result<(), BitTrackerError> {
    ...
    fn clear(&self) -> Result<bool, BitTrackerError> {
        OpenOptions::new()
            .write(true)
            .create(true)
            .truncate(true)
            .open(&self.path)
            .or(Err(BitTrackerError::OpenFileError))?;
        Ok(true)
    }
}
